...
module.exports = {
  data: new SlashCommandBuilder()
    ...
    .addSubcommand((sub) =>
      sub.setName('list').setDescription('List all stocks')
    )
    .addSubcommand((sub) =>
      sub.setName('price')
    ...
    .addSubcommand((sub) =>
      sub.setName('buy')
        .setDescription('Buy shares by quantity')
        .addStringOption((o) => o.setName('symbol').setDescription('Ticker').setRequired(true))
        .addNumberOption((o) => o.setName('shares').setDescription('Number of shares to buy').setRequired(true).setMinValue(1))
    )
    .addSubcommand((sub) =>
      sub.setName('sell')
        .setDescription('Sell shares by quantity')
        .addStringOption((o) => o.setName('symbol').setDescription('Ticker').setRequired(true))
        .addNumberOption((o) => o.setName('shares').setDescription('Number of shares to sell').setRequired(true).setMinValue(1))
    )
    .addSubcommand((sub) =>
  ...
  async execute(interaction) {
    ...
    if (sub === 'buy') {
      ...
      const maxCanBuy = Math.floor(user.stockBalance / asset.currentPrice);

      if (user.stockBalance < totalCost) {
        return interaction.editReply(
          `❌ Insufficient funds!\n\n` +
          `🛒 You want: **${shares} ${symbol}** shares = **${formatMoney(totalCost)}**\n` +
          `💰 Stock Account: **${formatMoney(user.stockBalance)}**\n` +
          `📊 Max you can buy: **${maxCanBuy} shares**\n\n` +
          `💡 Use \`/transfer\` to add funds to your Stock Account.`
        );
      }
      ...
      user.stockBalance = parseFloat((user.stockBalance - totalCost).toFixed(2));
      ...
      const embed = new EmbedBuilder()
        ...
        .addFields(
          { name: 'Stock', value: `${asset.name || symbol} (${symbol})`, inline: true },
          { name: 'Shares Bought', value: `${shares} shares`, inline: true },
          { name: 'Price Per Share', value: formatMoney(asset.currentPrice), inline: true },
    ...
    if (sub === 'sell') {
      ...
      if (!holding || holding.quantity < shares) {
        const maxSell = holding ? parseFloat(holding.quantity.toFixed(2)) : 0;
        const maxValue = parseFloat((maxSell * asset.currentPrice).toFixed(2));
        return interaction.editReply(
          `❌ Insufficient shares!\n\n` +
          `🛒 You want to sell: **${shares} ${symbol}** shares\n` +
          `💰 You own: **${maxSell} shares**\n` +
          `📊 Max sell value: **${formatMoney(maxValue)}**`
        );
      }
      ...
      user.stockBalance = parseFloat((user.stockBalance + usdValue).toFixed(2));
      ...
      const embed = new EmbedBuilder()
        ...
        .addFields(
          { name: 'Stock', value: `${asset.name || symbol} (${symbol})`, inline: true },
          { name: 'Shares Sold', value: `${shares} shares`, inline: true },
          { name: 'Price Per Share', value: formatMoney(asset.currentPrice), inline: true },
          ...
          { name: 'Remaining Holdings', value: `${holding.quantity} shares`, inline: true },
          { name: 'Stock Balance', value: formatMoney(user.stockBalance), inline: true }
        );
